...
describe("WorkplaceLabelingTaskPage", () => {
  ...
  it("surfaces validation errors and normalizes string assignment ids before batch submit", async () => {
    const user = userEvent.setup();
    ...
    taskService.submitMultiple.mockRejectedValue({
      response: {
        data: {
          errors: {
            assignmentIds: ["Assignment list cannot be empty."],
          },
        },
      },
    ...
    expect(toast.error).toHaveBeenCalledWith(
      "Assignment list cannot be empty.",
    );
  });
});
